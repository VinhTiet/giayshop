<?php 
   $id = $_GET['id'];
    ...
    $sql_str = "select * from brand where id=$id";
    $res = mysqli_query($conn, $sql_str);

    $brands = mysqli_fetch_assoc($res);
    if(isset($_POST['btnUpdate'])) {
        $name = $_POST['name'];
        $slug = strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/','-', $name)));
        $status = $_POST['status'];
        $sql_str = "update brand set name='$name', status='$status', slug='$slug' where id=$id";
                            ...
                            <form class="user" method="post" action="#">
                                ...
                                <div class="form-group">
                                    <input type="text" class="form-control form-control-user" id="name" name="name"
                                        placeholder="Tên Thương Hiệu" value="<?=$brands['name']?>">

                                </div>

                                <div class="form-group">
                                    <input type="text" class="form-control form-control-user" id="status" name="status"
                                        placeholder="Trạng Thái" value="<?=$brands['status']?>">
                                </div>
